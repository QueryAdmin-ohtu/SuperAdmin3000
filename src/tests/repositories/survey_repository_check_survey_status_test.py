...
class TestSurveyRepository(unittest.TestCase):
    ...
    def test_check_survey_status_where_no_survey_results_returns_correct(self):
        with self.app.app_context():
            survey_id = self.repo.get_survey(
                self.index_of_survey_without_results)[0]
    ...
    def test_check_survey_status_where_no_question_answers_returns_correct(self):
        with self.app.app_context():
            ...
            self.repo.create_question(
                "mock question one", survey_id, self.category_weights)

            check = self.repo.check_survey_status(survey_id)
            self.assertEqual(check[5], ["mock question one"])

            question_two_id = self.repo.create_question(
                "mock question two", survey_id, self.category_weights)
            check = self.repo.check_survey_status(survey_id)
            self.assertEqual(
                check[5], ["mock question one", "mock question two"])

            self.repo.create_answer(
                "mock answer to question two", 0, question_two_id)
            check = self.repo.check_survey_status(survey_id)
            self.assertEqual(check[5], ["mock question one"])
    ...
    def test_check_survey_status_where_one_question_without_categories_returns_correct(self):
        with self.app.app_context():
            ...
            self.repo.create_question(
                "mock question one", survey_id, '[{"category": "status category 1", "multiplier": 0.0}, {"category": "status category 2", "multiplier": 0.0}]')
            check = self.repo.check_survey_status(survey_id)
    ...
    def test_check_survey_status_where_multiple_question_without_categories_returns_correct(self):
        with self.app.app_context():
            ...
            self.assertTrue(check[6] == ["mock question one"])
            self.repo.create_question(
                "mock question two", survey_id, '[{"category": "status category 1", "multiplier": 0.0}, {"category": "status category 2", "multiplier": 0.0}]')
            check = self.repo.check_survey_status(survey_id)
            self.assertTrue(
                check[6] == ["mock question one", "mock question two"])
            self.deleteSurvey(survey_id)
    ...
    def test_check_survey_status_where_mix_of_questions_without_categories_returns_correct(self):
        with self.app.app_context():
            survey_id = self.createSurvey("questions_without_categories")
            self.repo.create_question(
                "mock question one", survey_id, '[{"category": "status category 1", "multiplier": 0.0}, {"category": "status category 2", "multiplier": 0.1}]')
            self.repo.create_question(
                "mock question two", survey_id, '[{"category": "status category 1", "multiplier": 0.0}, {"category": "status category 2", "multiplier": 0.0}]')
            self.repo.create_question("mock question three", survey_id,
                                      '[{"category": "status category 1", "multiplier": 1.0}, {"category": "status category 2", "multiplier": 0.0}]')
            check = self.repo.check_survey_status(survey_id)
    ...
    def test_check_survey_status_where_no_problems_present_returns_correct(self):
        with self.app.app_context():
            ...
            category_id = self.repo.create_category(
                survey_id, "status category 1", "Mock Description", [])
            ...
            question_id = self.repo.create_question(
                "Question with answers and so forth!", survey_id, '[{"category": "status category 1", "multiplier": 1.0}]')
            self.repo.create_answer("Answer to question", 1, question_id)
            check = self.repo.check_survey_status(survey_id)
            self.assertEquals(len(check), 9)
            self.assertEquals(
                check, ['green', False, False, [], False, [], [], [], {}])
            self.deleteSurvey(survey_id)
    ...
    def test_check_survey_status_returns_red(self):
        with self.app.app_context():
            ...
            self.assertEquals(
                self.repo.check_survey_status(survey_id)[0], "red")
            category_id = self.repo.create_category(
                survey_id, "status category 1", "Mock Description", [])
            self.assertEquals(
                self.repo.check_survey_status(survey_id)[0], "red")
            ...
            self.repo.create_answer("Answer to question", 1, question_id)
            self.assertEquals(
                self.repo.check_survey_status(survey_id)[0], "green")
            self.deleteSurvey(survey_id)
    ...
    def test_check_survey_status_returns_yellow(self):
        with self.app.app_context():
            ...
            category_id = self.repo.create_category(
                survey_id, "status category 1", "Mock Description", [])
            category_id_no_questions = self.repo.create_category(
                survey_id, "No questions", "Leads to yellow status", [])
            self.repo.create_survey_result(survey_id, "Magnificient!", 1.0)
            self.repo.create_category_result(category_id, "Magnificient!", 1.0)
            self.repo.create_category_result(
                category_id_no_questions, "Magnificient!", 1.0)
            question_id = self.repo.create_question(
                "Question with answers and so forth!", survey_id, '[{"category": "status category 1", "multiplier": 0.0}]')
            self.repo.create_answer("Answer to question", 1, question_id)

            self.assertEquals(
                self.repo.check_survey_status(survey_id)[0], "yellow")
            self.deleteSurvey(survey_id)
    ...
    def test_get_unrelated_categories_in_weights_returns_unrelated_category_names(self):
        ...
        result = self.repo.get_unrelated_categories_in_weights(
